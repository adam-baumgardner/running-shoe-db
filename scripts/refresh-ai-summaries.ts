...
async function main() {
  ...
  let failedCount = 0;
  const failedReleases: Array<{ id: string; label: string; error: string }> = [];
  ...
  for (const release of releaseRows) {
    try {
      ...
      failedReleases.push({
        id: release.id,
        label: `${release.brandName} ${release.versionName}`,
        error: error instanceof Error ? error.message : "Unknown error",
      });
      console.error("AI summary refresh failed for release", {
