...
interface RunDoctorsOfRunningImportParams {
  releaseId: string;
}

interface DoctorsOfRunningCandidate {
  sourceUrl: string;
  title: string;
  confidence: number;
}

interface DoctorsOfRunningArticle {
  sourceUrl: string;
  title: string;
  summary: string;
  body: string;
...
export async function runDoctorsOfRunningImport({
  releaseId,
}: RunDoctorsOfRunningImportParams): Promise<CrawlExecutionResult> {
  const db = getDb();
  const reviewSourceId = await getDoctorsOfRunningSourceId();

  if (!reviewSourceId) {
    throw new Error("Doctors of Running review source is not configured.");
  }
  ...
  const release = await db
    ...
    .innerJoin(
      ...
      and(
        ...
        eq(crawlSources.importerKey, doctorsOfRunningImporter.key),
      ),
    )
  ...
  if (!selected) {
    throw new Error("Unable to resolve release or crawl source for Doctors of Running.");
  }

  const query = doctorsOfRunningImporter.normalizeQuery({
  ...
  const run = await db
    ...
    .values({
      ...
      startedAt: new Date(),
      metadata: {
        importer: doctorsOfRunningImporter.key,
  ...
  try {
    const candidates = await discoverDoctorsOfRunningCandidates({
      brandName: selected.brandName,
    ...
    for (const candidate of candidates) {
      ...
      const article = await fetchDoctorsOfRunningArticle(candidate);
      const authorId = await getOrCreateReviewAuthor(selected.sourceId, "Doctors of Running");
    ...
    await db
      ...
      .set({
        ...
        metadata: {
          ...
          failureStage: null,
          noHitReason: candidates.length === 0 ? "no matching doctors of running article found" : null,
        },
      })
...
async function getDoctorsOfRunningSourceId() {
  const db = getDb();
  const source = await db.query.reviewSources.findFirst({
    where: eq(reviewSources.slug, doctorsOfRunningImporter.sourceSlug),
  });

  return source?.id ?? null;
}

async function discoverDoctorsOfRunningCandidates({
  brandName,
  versionName,
  query,
}: {
  ...
  const searchUrl = doctorsOfRunningImporter.buildSearchUrl({
    brandName,
  ...
  if (!response.ok) {
    throw new Error(`Doctors of Running search request failed with ${response.status}`);
  }
  ...
  return $("a")
    ...
    .map((element) => {
      ...
      const sourceUrl = normalizeDoctorsUrl(href);
      if (
        !sourceUrl ||
        seen.has(sourceUrl) ||
        !sourceUrl.includes("doctorsofrunning.com") ||
    ...
    .filter((candidate): candidate is DoctorsOfRunningCandidate => Boolean(candidate))
    .slice(0, 5);
}

async function fetchDoctorsOfRunningArticle(
  candidate: DoctorsOfRunningCandidate,
): Promise<DoctorsOfRunningArticle> {
  ...
  if (!response.ok) {
    throw new Error(`Doctors of Running article request failed with ${response.status}`);
  }
  ...
  const summary = summarizeParts(
    [
      ...
      extractDoctorsBodyText($),
    ],
    420,
  );
  const body = extractDoctorsBodyText($) || summary || title;

  return {
    sourceUrl: candidate.sourceUrl,
    title: isGenericDoctorsTitle(title) ? humanizeDoctorsSlug(candidate.sourceUrl) : title,
...
function isGenericDoctorsTitle(value: string) {
  const normalized = normalizeSearchText(value);
  return normalized === "doctors of running" || normalized.length < 6;
}

function humanizeDoctorsSlug(sourceUrl: string) {
  const slug = sourceUrl
    .replace(/^https:\/\/www\.doctorsofrunning\.com\//, "")
    .replace(/\.html$/, "")
    .split("/")
    .pop();

  if (!slug) {
    return "Doctors of Running review";
  }

  return cleanText(slug.replace(/-/g, " "));
}

function extractDoctorsBodyText($: cheerio.CheerioAPI) {
  const containers = [
    $(".post-body").first(),
    $("article").first(),
    $("main").first(),
...
function normalizeDoctorsUrl(value: string) {
  try {
    return new URL(value, "https://www.doctorsofrunning.com").toString();
  } catch {
    return null;
  }
}
...
function getCandidateConfidence(haystack: string, normalizedQuery: string) {
  ...
  if (haystack.includes("doctorsofrunning.com")) {
    score += 0.04;
  }
